...
async function processFrameworksInBatches() {
  console.log('📚 Starting framework enhancement process (batch mode)...\n');
  ...
  for (let i = 0; i < allFrameworks.length; i++) {
    ...
    try {
      console.log(`\n[${i + 1}/${allFrameworks.length}] 🔄 Processing: ${fw.subjectName} (Framework ID: ${fw.id})`);
      ...
      if (hasKeyConceptsAlready) {
        console.log('⏭️  Already has key_concepts, skipping...');
        skippedCount++;
        continue;
      }
